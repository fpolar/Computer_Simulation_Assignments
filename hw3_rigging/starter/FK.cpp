    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
  ...
  // Also useful are the Mat3d and RigidTransform4d classes defined in the Vega folder.
	...
	// for every joint in the jointParents/jointUpdateOrder arrays:
	for (int i = 0; i < jointUpdateOrder.size(); i++) {
		...
		// Compute rotations
		euler2Rotation(eulerAngles[jointUpdateOrder[i]], initR, rotateOrders[jointUpdateOrder[i]]);
		RMMatrix = Mat3d(initR);
		euler2Rotation(jointOrientationEulerAngles[jointUpdateOrder[i]], initR, RotateOrder::XYZ);
		ROMatrix = Mat3d(initR);

		// Compute local joint transforms
		...
		// Recursively compute the globalTransforms using jointUpdateOrder and jointParents
		currJointOrder = jointUpdateOrder[i];

		// If joint is root
		if (jointParents[currJointOrder] == -1) {
			globalTransforms[currJointOrder] = localTransforms[currJointOrder];
		}else {
			globalTransforms[currJointOrder] = globalTransforms[jointParents[currJointOrder]] * localTransforms[currJointOrder];
		}
	}
}
