...
{
	vector<Vec4d> newVertexPositions(numMeshVertices);
	vector<Vec4d> restVertexPositions(numMeshVertices);
	for (int i = 0; i < numMeshVertices; i++) {
		newVertexPositions[i] = Vec4d(0, 0, 0, 0);
		restVertexPositions[i] = Vec4d(restMeshVertexPositions[3 * i], restMeshVertexPositions[3 * i + 1], restMeshVertexPositions[3 * i + 2], 1.0);
	}
	...
	for (int i = 0; i < numMeshVertices; i++) {
		for (size_t j = 0; j < numJointsInfluencingEachVertex; j++) {
			newVertexPositions[i] += meshSkinningWeights[i * numJointsInfluencingEachVertex + j] * jointSkinTransforms[meshSkinningJoints[i * numJointsInfluencingEachVertex + j]] * restVertexPositions[i];
		}
